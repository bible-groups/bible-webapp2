  ...
  <div class="min-h-screen">
    <div class="container mx-auto px-4 py-8">
      ...
      <div class="text-center mb-8">
        <h1 class="text-[1.8rem] xl:text-3xl font-bold text-gray-800 mb-4">말씀 안내</h1>
      ...
      <div v-if="viewMode === 'add'" class="max-w-6xl mx-auto mb-8 border border-gray-200 rounded-lg">
        <div class="bg-white rounded-lg shadow-md p-6">
          <h2 class="text-xl font-semibold text-gray-800 mb-6">새 안내 추가</h2>
          
          <form @submit.prevent="addNewGuide" class="space-y-4">
            <!-- 안내 제목 -->
            <div>
              <label class="block text-sm font-medium text-gray-700 mb-2">안내 제목</label>
              <input 
            ...
            <!-- 안내 내용 -->
            <div>
              <label class="block text-sm font-medium text-gray-700 mb-2">안내 내용</label>
              <textarea 
      ...
      <div v-if="!loading && allFilteredGuides.length === 0" class="max-w-6xl mx-auto text-center py-12">
        ...
        <p class="text-gray-500 mb-4">
          {{ searchTerm ? '검색 조건에 맞는 안내가 없습니다.' : '등록된 안내가 없습니다.' }}
        </p>
        <button 
          v-if="searchTerm"
          @click="clearFilters"
    ...
    <div v-if="modalOpen" class="fixed inset-0 bg-black/50 flex items-center justify-center z-100 p-4" @click.self="closeModal">
      <div class="bg-white rounded-lg shadow-xl w-full lg:w-6xl max-h-[90vh] overflow-hidden flex flex-col">
        ...
        <div class="p-6 overflow-y-auto" style="height: 60vh;">
          <!-- 설명 (편집 가능) -->
          <div class="mb-6">
            <label class="block text-base font-bold text-gray-700 mb-2">안내 내용</label>
            <textarea 
              v-model="editBody"
...
const loadGuidesFromFirebase = async () => {
  try {
    ...
    const q = query(guidesCollection, orderBy('createAt', 'desc')) // 최신순 정렬
    ...
    console.log('Firebase에서 안내 데이터 로드 완료:', guides.value.length, '개')
  } catch (error) {
    console.error('안내 데이터 로드 실패:', error)
...
const updateGuide = async () => {
  ...
  try {
    ...
    const guideDoc = doc($db, 'guide', selectedGuide.value.id)
    ...
    console.log('안내 수정 완료')
    alert('안내가 성공적으로 수정되었습니다.')
    
  } catch (error) {
    console.error('안내 수정 실패:', error)
    alert('안내 수정 중 오류가 발생했습니다.')
  } finally {
    updating.value = false
  }
}
...
const deleteGuide = async () => {
  ...
  try {
    ...
    console.log('안내 삭제 완료')
    alert('안내가 성공적으로 삭제되었습니다.')
    
    // 모달 닫기
    closeModal()
    
  } catch (error) {
    console.error('안내 삭제 실패:', error)
    alert('안내 삭제 중 오류가 발생했습니다.')
  } finally {
    deleting.value = false
  }
}
...
const addNewGuide = async () => {
  try {
    ...
    guides.value.unshift(newGuideData) // 최신순으로 맨 앞에 추가
    ...
    console.log('새 안내 추가 완료')
    alert('안내가 성공적으로 추가되었습니다.')
    
  } catch (error) {
    console.error('안내 추가 실패:', error)
    alert('안내 추가 중 오류가 발생했습니다.')
  } finally {
    adding.value = false
  }
}
