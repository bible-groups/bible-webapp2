  ...
  <div class="min-h-screen">
    <div class="container mx-auto px-4 py-8">
      ...
      <div class="text-center mb-8">
        <h1 class="text-[1.8rem] xl:text-3xl font-bold text-gray-800 mb-4">성서이해</h1>
        <p class="text-gray-600">성경을 깊이 있게 이해하고 배울 수 있는 콘텐츠를 제공합니다.</p>
      </div>
...
// Firebase에서 데이터 로드
onMounted(async () => {
  console.log('[PAGE] understand mounted')
...
const loadPostsFromFirebase = async () => {
  try {
    ...
    // category가 '성서이해'인 데이터만 가져오기, createAt 기준 최신순 정렬
    const q = query(
      blogsCollection, 
      where('category', '==', '성서이해'),
    ...
    console.log('Firebase에서 성서이해 데이터 로드 완료:', posts.value.length, '개')
  } catch (error) {
