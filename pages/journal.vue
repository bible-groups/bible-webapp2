  ...
  <div class="min-h-screen">
    <div class="container mx-auto px-4 py-8">
      ...
      <div class="text-center mb-8">
        <h1 class="text-[1.8rem] xl:text-3xl font-bold text-gray-800 mb-4">일기장</h1>
        <p class="text-gray-600">하나님과 함께한 하루를 간단히 기록하세요</p>
      </div>
      ...
      <div v-if="viewMode === 'add'" class="max-w-6xl mx-auto mb-8 border border-gray-200 rounded-lg">
        <div class="bg-white rounded-lg shadow-md p-6">
          <h2 class="text-xl font-semibold text-gray-800 mb-6">새 일기 추가</h2>
            ...
            <div class="flex items-start space-x-4">
              ...
              <div class="flex-shrink-0 mt-1">
                <div class="w-10 h-10 bg-indigo-300 rounded-full flex items-center justify-center">
                  <!-- 추천 아이콘: 일기장 느낌의 책 아이콘 -->
                  <i class="fa fa-book text-white text-lg fa-fw"></i>
                </div>
              </div>
              <!-- 내용 -->
              <div class="flex-1 min-w-0">
                <div class="flex items-center justify-between mb-2">
                  <h3 class="text-md xl:text-lg font-semibold text-gray-900 truncate">{{ entry.title }}</h3>
                  <div class="flex items-center space-x-2 ml-4">
                    <span class="text-sm text-gray-500">{{ entry.createdAt }}</span>
                    <span class="text-sm text-gray-400 hidden sm:block">{{ entry.author }}</span>
                  </div>
                </div>
                <p class="text-gray-600 text-sm mb-3 line-clamp-2">{{ entry.body }}</p>
                <div class="flex items-center justify-between">
                  <div></div>
                  <button class="text-indigo-600 hover:text-indigo-800 text-sm font-medium flex items-center space-x-1" @click.stop="openModal(entry)">
      ...
      <!-- 페이지네이션 -->
      <div v-if="allFilteredEntries.length > 0" class="max-w-6xl mx-auto flex justify-center mt-8">
      ...
      <!-- 데이터가 없을 때 -->
      <div v-if="!loading && allFilteredEntries.length === 0" class="max-w-6xl mx-auto text-center py-12">
        <div class="text-gray-400 mb-4">
          <svg class="w-16 h-16 mx-auto" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9.172 16.172a4 4 0 015.656 0M9 12h6m-6-4h6m2 5.291A7.962 7.962 0 0112 15c-2.34 0-4.515-.751-6.281-2.02M12 15c2.34 0 4.515-.751 6.281-2.02M12 3v12"></path></svg>
        </div>
        <p class="text-gray-500">작성된 일기가 없습니다.</p>
      </div>
    </div>
      ...
      <div class="bg-white rounded-lg shadow-xl w-full lg:w-6xl max-h-[90vh] overflow-hidden flex flex-col">
        ...
        <div class="flex justify-between items-center p-6 border-b border-gray-200">
          <div class="flex-1 pr-4">
            <input v-model="editTitle" class="w-full text-2xl font-bold text-gray-800 bg-transparent border-none outline-none focus:bg-gray-50 focus:px-3 focus:py-2 focus:border focus:border-gray-300 focus:rounded-lg transition-all duration-200 hover:bg-gray-50" placeholder="일기 제목을 입력하세요">
          </div>
          <div class="flex items-center space-x-4 flex-shrink-0">
            <span class="text-sm text-gray-500">{{ selectedEntry.createdAt }}</span>
        ...
        <div class="p-6 overflow-y-auto" style="height: 60vh;">
          <div class="mb-6">
            <label class="block text-base font-bold text-gray-700 mb-2">일기 내용</label>
            <textarea v-model="editBody" :rows="calculateRows(editBody)" class="w-full text-gray-700 bg-transparent border-none outline-none focus:bg-gray-50 focus:px-3 focus:py-2 focus:border focus:border-gray-300 focus:rounded-lg resize-none transition-all duration-200 leading-relaxed hover:bg-gray-50" placeholder="일기 내용을 입력하세요"></textarea>
          </div>
        </div>
        <!-- 모달 하단 -->
        <div class="p-6 border-t border-gray-200">
          <div class="flex justify-center space-x-4">
            <button @click="updateEntry" :disabled="updating" class="w-12 h-12 sm:w-auto sm:h-auto sm:px-6 sm:py-2 bg-indigo-500 text-white rounded-lg hover:bg-indigo-400 disabled:opacity-50 disabled:cursor-not-allowed transition-colors flex items-center justify-center sm:space-x-2 cursor-pointer">
            ...
            <button @click="deleteEntry" :disabled="deleting" class="w-12 h-12 sm:w-auto sm:h-auto sm:px-6 sm:py-2 bg-red-500 text-white rounded-lg hover:bg-red-400 disabled:opacity-50 disabled:cursor-not-allowed transition-colors flex items-center justify-center sm:space-x-2 cursor-pointer">
            ...
            <button @click="closeModal" class="w-12 h-12 sm:w-auto sm:h-auto sm:px-6 sm:py-2 bg-gray-500 text-white rounded-lg hover:bg-gray-400 transition-colors flex items-center justify-center sm:space-x-2 cursor-pointer">
...
// 데이터 로드
onMounted(async () => {
  console.log('[PAGE] journal mounted')
...
const loadEntriesFromFirebase = async () => {
  try {
    ...
    console.log('Firebase에서 일기 데이터 로드 완료:', entries.value.length, '개')
  } catch (error) {
...
const allFilteredEntries = computed(() => {
  ...
  if (searchTerm.value) {
    ...
    filtered = filtered.filter(e =>
      ...
      (e.author && e.author.toLowerCase().includes(s))
    )
  }
  return filtered
})
...
// 페이지네이션 계산
const totalPages = computed(() => Math.ceil(allFilteredEntries.value.length / itemsPerPage.value))
const totalGroups = computed(() => Math.ceil(totalPages.value / pagesPerGroup.value))
const startIndex = computed(() => (currentPage.value - 1) * itemsPerPage.value)
const endIndex = computed(() => startIndex.value + itemsPerPage.value)
const filteredEntries = computed(() => allFilteredEntries.value.slice(startIndex.value, endIndex.value))
...
const calculateRows = (text) => {
  ...
  for (let line of lines) totalRows += Math.floor(line.length / 80)
...
const addNewEntry = async () => {
  try {
    ...
    entries.value.unshift(newData)
    newEntry.value = { title: '', body: '' }
    viewMode.value = 'list'
    alert('일기가 성공적으로 추가되었습니다.')
  } catch (e) {
    console.error('일기 추가 실패:', e)
    alert('일기 추가 중 오류가 발생했습니다.')
  } finally {
    adding.value = false
  }
}
...
const updateEntry = async () => {
  ...
  try {
    ...
    const entryDoc = doc($db, 'journal', selectedEntry.value.id)
    await updateDoc(entryDoc, { title: editTitle.value, body: editBody.value })
    // 로컬 업데이트
    const idx = entries.value.findIndex(e => e.id === selectedEntry.value.id)
    if (idx !== -1) entries.value[idx] = { ...entries.value[idx], title: editTitle.value, body: editBody.value }
    selectedEntry.value = { ...selectedEntry.value, title: editTitle.value, body: editBody.value }
