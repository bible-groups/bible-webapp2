  ...
  <div class="min-h-screen">
    <div class="container mx-auto px-4 py-8">
      ...
      <div class="text-center mb-8">
        <h1 class="text-[1.8rem] xl:text-3xl font-bold text-gray-800 mb-4">말씀새김</h1>
        <p class="text-gray-600">말씀을 깊이 새기고 묵상하는 공간입니다.</p>
      </div>
        ...
        <div class="bg-white rounded-lg shadow-md p-6">
          ...
          <form @submit.prevent="addNewPost" class="space-y-4">
            ...
            <div>
              ...
              >
              <p class="text-xs text-red-500 mt-4 mb-10 pl-3">* 쉼표(,)로 구분하여 여러 키워드를 입력할 수 있습니다 (예: 사랑, 믿음, 소망)</p>
            </div>
      ...
      <div class="bg-white rounded-lg shadow-xl w-full lg:w-6xl max-h-[90vh] overflow-hidden flex flex-col">
        ...
        <div class="p-6 border-t border-gray-200">
          <div class="mb-5 flex justify-center"> 
            <!-- 관리자: 키워드 편집 영역과 표시 영역을 나란히 배치 -->
            <div v-if="role === 'admin'" class="flex gap-2 items-center max-w-4xl w-full">
              <!-- 관리자용 키워드 편집 영역 -->
              <div class="flex items-center gap-1 flex-[2]">
                <label class="text-sm font-medium text-gray-700 whitespace-nowrap mr-2" style="min-width: 60px;">키워드</label>
                <input 
                  v-model="editKeywordsInput"
                  type="text"
                  @input="parseEditKeywords"
              ...
              <!-- 키워드 표시 -->
              <div class="flex items-center justify-center min-h-[50px] flex-[1]">
                <template v-if="editKeywords && editKeywords.length > 0">
                  <div class="flex flex-wrap gap-2 justify-center">
                    <button 
                      v-for="(keyword, index) in editKeywords" 
                      :key="index"
                      @click="openKeywordModal(keyword)"
                      class="inline-flex items-center bg-indigo-100 text-indigo-800 text-xs px-3 py-2 rounded-full hover:bg-indigo-200 transition-colors cursor-pointer"
                    >
                      {{ keyword }}
                      <span class="ml-1 text-indigo-600" @click.stop="removeEditKeyword(index)">
                ...
                <template v-else>
                  <span class="text-gray-400 text-sm">키워드를 입력하세요</span>
                </template>
              </div>
            </div>
    ...
    <!-- 키워드 상세 모달 (기존 모달 위에 표시) -->
    <div v-if="keywordModalOpen" class="fixed inset-0 flex items-center justify-center z-[110] p-4 pointer-events-none">
      <div class="pointer-events-auto" @click.self="closeKeywordModal">
      <div class="bg-white rounded-lg shadow-xl w-full lg:w-6xl max-h-[90vh] overflow-hidden flex flex-col">
        <div class="p-6 border-b border-gray-200">
          <div class="flex justify-end mb-4">
            <button 
          ...
          <div class="text-center mb-5">
            <h2 class="text-2xl font-bold text-gray-800">{{ keywordModalData.title }}</h2>
            <p class="text-sm text-gray-500 italic mt-1">Enlightenment</p>
          </div>
        </div>

        <div class="px-6 py-15 overflow-y-auto" style="height: 64vh;">
          <div v-if="keywordLoading" class="flex justify-center items-center py-12">
          ...
          <div v-else class="text-gray-700 leading-relaxed whitespace-pre-wrap">
            {{ keywordModalData.body }}
          </div>
        </div>

        <div class="p-6 border-t border-gray-200">
          <div :class="['flex py-7', role === 'admin' ? 'justify-end' : 'justify-end']">
            <button
              @click="closeKeywordModal"
...
const newPost = ref({ title: '', body: '', keywords: [] })
...
const keywordModalData = ref({ title: '', body: '' })
...
const loadPostsFromFirebase = async () => {
  try {
    ...
    const q = query(blogsCollection, where('category', '==', '말씀새김'), orderBy('createAt', 'desc'))
    ...
    console.log('Firebase에서 말씀새김 데이터 로드 완료:', posts.value.length, '개')
  } catch (error) {
...
const openModal = (post) => {
  ...
  editKeywords.value = post.keywords || []
  editKeywordsInput.value = (post.keywords || []).join(', ')
...
const parseKeywords = () => {
  if (keywordsInput.value.trim()) {
    ...
    // 중복 제거
    newPost.value.keywords = [...new Set(keywords)]
  } else {
    newPost.value.keywords = []
  }
}

const parseEditKeywords = () => {
  if (editKeywordsInput.value.trim()) {
    const keywords = editKeywordsInput.value
      .split(',')
      .map(keyword => keyword.trim())
      .filter(keyword => keyword.length > 0)
    
    editKeywords.value = [...new Set(keywords)]
  } else {
    editKeywords.value = []
  }
}

const removeEditKeyword = (index) => {
  editKeywords.value.splice(index, 1)
  editKeywordsInput.value = editKeywords.value.join(', ')
}

const openKeywordModal = async (keyword) => {
  try {
    keywordLoading.value = true
    keywordModalOpen.value = true
    keywordModalData.value = { title: keyword, body: '로딩 중...' }
    
    const { $db } = useNuxtApp()
    const { collection, getDocs, query, where } = await import('firebase/firestore')
    const conceptsCollection = collection($db, 'concepts')
    const q = query(conceptsCollection, where('title', '==', keyword))
    const querySnapshot = await getDocs(q)
...
const closeKeywordModal = () => {
  ...
  keywordModalData.value = { title: '', body: '' }
}
...
const addNewPost = async () => {
  try {
    ...
    const docRef = await addDoc(blogsCollection, {
      ...
      category: '말씀새김',
      author: '관리자',
      keywords: newPost.value.keywords,
    ...
    newPost.value = { title: '', body: '', keywords: [] }
    keywordsInput.value = ''
    viewMode.value = 'list'
