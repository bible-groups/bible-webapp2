  ...
  <div class="min-h-screen">
    <div class="container mx-auto px-4 py-8">
      ...
      <div class="text-center mb-8">
        <h1 class="text-[1.8rem] xl:text-3xl font-bold text-gray-800 mb-4">개인 묵상</h1>
            ...
            <div>
              ...
              <div class="flex items-center space-x-6">
                <label class="flex items-center">
                  <input 
                    type="radio" 
                    v-model="newMeditation.share" 
                    :value="true"
                    class="w-4 h-4 text-indigo-600 bg-gray-100 border-gray-300 focus:ring-indigo-500"
                  >
                  <span class="ml-2 text-sm font-medium text-gray-700">공개</span>
                </label>
                <label class="flex items-center">
                  <input 
                    type="radio" 
                    ...
                    class="w-4 h-4 text-indigo-600 bg-gray-100 border-gray-300 focus:ring-indigo-500"
                  >
                  <span class="ml-2 text-sm font-medium text-gray-700">비공개</span>
                </label>
              </div>
            </div>
      ...
      <!-- 페이지네이션 -->
      <div v-if="allFilteredMeditations.length > 0" class="max-w-6xl mx-auto flex justify-center mt-8">
      ...
      <!-- 데이터가 없을 때 -->
      <div v-if="!loading && allFilteredMeditations.length === 0" class="max-w-6xl mx-auto text-center py-12">
    ...
    <div v-if="modalOpen" class="fixed inset-0 bg-black/50 flex items-center justify-center z-100 p-4" @click.self="closeModal">
      <div class="bg-white rounded-lg shadow-xl w-full lg:w-6xl max-h-[90vh] overflow-hidden flex flex-col">
        ...
        <div class="p-6">
          ...
          <!-- 공개/비공개 설정 (편집 가능) -->
          <div class="mb-4">
            <label class="block text-base font-bold text-gray-700 mb-2">공개 설정</label>
            <div class="flex items-center space-x-6">
              <label class="flex items-center">
                <input 
                  type="radio" 
...
const loadMeditationsFromFirebase = async () => {
  try {
    ...
    const currentUserName = currentUser.displayName || '익명'
    // console.log('현재 사용자:', currentUserName)
    // console.log('사용자 객체:', currentUser)
    ...
    const allMeditations = querySnapshot.docs.map(doc => {
      ...
      return {
        ...
        prayer: data.prayer || '',
        author: data.author || '',
        share: data.share !== undefined ? data.share : true, // 기본값은 true (공개)
        createdAt: formatFirebaseDate(data.createdAt),
        createdAtRaw: data.createdAt
      }
    })
    
    // console.log('모든 묵상 데이터:', allMeditations)
    // console.log('검색할 사용자명:', currentUserName)
    
    // 클라이언트에서 현재 사용자의 글만 필터링
    meditations.value = allMeditations.filter(meditation => {
      const isMatch = meditation.author === currentUserName
      console.log(`묵상 ${meditation.id}: author="${meditation.author}" === "${currentUserName}" = ${isMatch}`)
      return isMatch
    })
    
    console.log('필터링된 사용자 묵상 데이터:', meditations.value.length, '개')
    console.log('결과:', meditations.value)
...
const openModal = (meditation) => {
  ...
  editShare.value = meditation.share !== undefined ? meditation.share : true
...
const addNewMeditation = async () => {
  try {
    ...
    const docRef = await addDoc(meditationsCollection, {
      ...
      author: user.value?.displayName || '익명',
      share: newMeditation.value.share,
